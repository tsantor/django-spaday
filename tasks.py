...
def coverage(c):
    ...
    """
    c.run("coverage run --source django_spaday runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/django-perm-filter.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ django_spaday")
...
def lint(c):
    ...
    """
    c.run("flake8 django_spaday tests")
...
def release(c, bumpsize=''):
    ...
    import django_spaday
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=django_spaday.__version__))
    c.run("git push --tags")
